...
Default is viewer-only (no hardware). Opt in explicitly with ``--ditto`` and/or
``--sharpa``.
...
Usage (from sharpa_teleop repo root):
    ...
    python sharpa_teleop/viz/view_teleop.py --ditto             # ditto only
    python sharpa_teleop/viz/view_teleop.py --sharpa            # sharpa only
...
def _strip_view_flags() -> ViewFlags:
    ...
    flags.sharpa_hardware = flags.sharpa_hardware and flags.show_sharpa
    return flags
...
def main() -> None:
    ...
    hardware = None
    force_render_controller: DittoSharpaTeleop | None = None
    ...
    if flags.ditto_hardware:
        if is_force_render_config(cfg):
            ...
            force_render_controller = DittoSharpaTeleop(
