"""Ditto–Sharpa bilateral teleop with force rendering.

``DittoSharpaTeleop`` drives the Ditto leader in current mode and the Sharpa
follower in position mode. Per finger (from ``hand_config.control.fingers``):

1. read the Ditto leader → leader ``q``
2. drive Sharpa via retarget IK and/or a direct joint map
3. build a follower torque signal (tactile / estimate / measured / blend)
4. feed synthetic follower currents through ditto's ``CurrentController``
   force rendering → leader current
...
from teleop.engine import DittoSharpaEngine  # noqa: E402
...
#   force:    "measured" (Sharpa joint torque, direct map)
...
# (-k·follower_current), which is tuned for a directly-measured follower torque.
# For task-space estimate/tactile the follower signal is the estimated leader
# joint torque we want to *reproduce* (τ = Jᵀ·F). Negating the synthetic current
# makes the law render +τ (resist the user into the contact) instead of -τ.
# Flip this if the felt force is inverted.
FORCE_RENDER_FEEDBACK_SIGN = -1.0


class DittoSharpaTeleop:
    """Ditto leader (current) ↔ Sharpa follower with retarget/joint position + force rendering."""
    ...
    def __init__(
        ...
        *,
        ...
        if leader_cfg.mode != "current":
            raise ValueError(
                "DittoSharpaTeleop requires leader mode 'current', "
                f"got {leader_cfg.mode!r} "
                "(e.g. hand_config=ditto_2f_tactile or joint/sharpa_3dof_index)."
            )
