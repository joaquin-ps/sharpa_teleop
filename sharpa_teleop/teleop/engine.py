"""Core Ditto → Sharpa teleop loop.

``DittoSharpaEngine`` owns kinematic retargeting plus optional hardware session
references, and exposes the steady-state operations shared by the GUI viewer and
headless runner:

- ``poll_leader()``               read Ditto leader → leader ``q``
- ``retarget()``                  Ditto ``q`` → Sharpa ``q`` (and stream to follower)
- ``estimate_force_feedback()``   contact force from ``force_source`` → pad forces
  and would-be leader joint torques (observe-only)

Session lifecycle (start/stop) stays with the caller.
"""
...
class ForceFeedbackSample:
    """One observe-only force-feedback estimate (no hardware command)."""
...
class DittoSharpaEngine:
    """Ditto → Sharpa teleop core: poll leader, retarget, estimate force feedback."""
